...
def generate_complete_binary_tree(num_leaf, dim_out):
    leaf_action = [
        0
    ] * dim_out  # info: number of controllers is same as number of leaf nodes
    ...
    while stack:
        ...
        if len(left_parents) + len(right_parents) >= height:  # Leaf node
            ...
            stack.append((right_child, left_parents.copy(), right_parents + [node]))
            stack.append((left_child, left_parents + [node], right_parents.copy()))
...
def genDT(dim_in=784, dim_out=10, num_leaf=8, custom_leaf=None):
    ...
    if custom_leaf:
        ...
        init_leaves = generate_complete_binary_tree_custom(
            num_leaf=num_leaf, dim_out=dim_out, custom_leaf_actions=custom_leaf
        )
...
class STEFunction(torch.autograd.Function):
    ...
    def forward(ctx, input):
        ...
        )  # Find the maximum value and its index across each sample in the batch
        output = torch.zeros_like(input)  # Initialize output tensor with zeros
        output.scatter_(
            1, max_indices, 1.0
        )  # Set 1 at the index of the maximum value for each sample
        # get softmax output
    ...
    def backward(ctx, grad_output, method="identity"):
        ...
        if method == "softmax":
            (
                output,
                output_hard,
            ) = ctx.saved_tensors  # Retrieve the saved output from forward pass
            grad_input = (
                grad_output * output * (1 - output)
            )  # Propagate gradient through the softmax function
...
class DTSemNet(nn.Module):
    def __init__(
        ...
        self.out_features = len(
            self.B
        )  # number of nodes in layer1 = number of nodes in DT
        ...
        if len(self._over_param) == 0:
            ...
            if wt_init:
                ...
                self.linear1.weight.data = (
                    self.init_weights
                )  # intialize weights from DT weights
            ...
            self.linear1 = [
                nn.Linear(a, b)
                for a, b in zip([in_dim] + mid_nodes, mid_nodes + [int_nodes])
            ]
            with torch.no_grad():
                [
                    nn.init.zeros_(x.bias)
                    for x in self.linear1
        ...
        self.linear2 = nn.Linear(2 * self.out_features, self.num_leaves, bias=False)
        ...
        if self.is_regression:
            ...
            if self.linear_control:
                ...
                if reg_hidden > 0:
                    ...
                    self.regression_layer = nn.Sequential(hidden_layer, output_layer)

                else:
                    self.regression_layer = nn.Linear(
                        self.input_dim, t_out_dim, bias=True
                    )
    ...
    def forward(
        ...
        if not isinstance(in_x, torch.Tensor):
            in_x = torch.tensor(in_x, dtype=torch.float32)
        ...
        if self.is_regression:  # for regression
            ...
            if self.custom_leaf_unique:
                ...
                expanded_output = torch.zeros(
                    batch_size, expanded_dim, dtype=x.dtype, device=x.device
                )
    ...
    def leaves_to_weight(self, init_leaves, num_nodes):
        ...
        for ix, leaf in enumerate(init_leaves):
            ...
            A[ix] = np.argmax(
                leaf[-1]
            )  # action associated with leaf 0: Left and 1: Right
